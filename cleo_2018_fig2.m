...
per_data = zeros(lenx,length(per)-199-50);
roisize=1000;
f_roi = zeros(lenx,2*roisize);
per_data_roi = zeros(lenx,2*roisize);
...
%     per_temp = per_temp(1:2000);
    ...
    [Y, I] = max(per_data(i,1:end));
    
    solutions_per(i) = f_temp(I);
% %     solutions_rb(i) = frequency_rb(wmeas(i,:),1/(tmeas(2)-tmeas(1)));


    f_roi(i,:) = f_temp(I-roisize:I+roisize-1);
    per_temp_roi(i,:) = per_temp(I-roisize:I+roisize-1);
    

    solution_lse(i,:) = pnoise_fit_data((per_temp_roi(i,:)),f_roi(i,:),[I Y]);
...
% f_roi(1) + 
    ...
    solutions_lse(i) = f_roi(i,1)+solution_lse(i,1)*(f_roi(i,2)-f_roi(i,1));
...
plot(f_temp/1e6,30+10*log10(averaged_per),'color',cb);
plot(f_roi(1,:)/1e6,30+10*log10(squeeze(mean(percollection(5,1,:,:),3))),'color',co);
...
plot(f_roi(1,:),10*log10(pnoise_model_data(solution_lse(1,:),f_roi(1,:))));
