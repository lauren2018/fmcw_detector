...
function paramsF = MLEwG (freq, perin, params)
...
% -1 * SOM Eq (49)
datafun = @(params)(sum (sum ((expected (freq,params))))...
                        -sum (sum (data.*log (expected (freq,params)))));
options = optimset ('MaxFunEvals', 10000, 'MaxIter', 10000, 'TolFun', 1e-7);
% fminsearch performs the multivariable minimization
[paramsF,fval,exitflag,output]  = fminsearch (datafun, params, options);
...
function E = expected (xdata,params)
% The expected counts per pixel. (SOM Eq. 12)
    E = 10*log10( params(2)*1e-9 ./ ( 1 + (pi * (params(2)*1e-9) * ( xdata - params(1)*(xdata(2)-xdata(1)) ) ).^2 ) );
