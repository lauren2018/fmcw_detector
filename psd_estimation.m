...
std_est_periodogram_avg = zeros(length(Prx),length(distance));
std_est_periodogram_avg_lse = zeros(length(Prx),length(distance));
std_est_periodogram_avg_mle = zeros(length(Prx),length(distance));
...
%             x(j,i,:)=padarray(vout.Data(end/10:end-1),padm*N,0,'both');
%             x_correct(j,i,:)=padarray(vout1.Data(end/10:end-1),padm*N,0,'both');
            ...
            x_windowed_correct(j,i,:)=padarray(cat(1,zeros(round(N/10),1),vout1.Data(round(N/10)+1:round(N/10)+rightlength(j)),zeros(round(9*N/10)-rightlength(j),1)),padm*N,0,'both');
...
%             tempx_correct = squeeze(x_correct(j,i,:));
            ...
            for l=1:avgnum
                [per_avg(l,:),F_avg] = periodogram(tempx_windowed(((l-1)*end/avgnum)+1:((l)*end/avgnum)),window(@rectwin,length(tempx_windowed(1:end/avgnum))),length(tempx_windowed(1:end/avgnum)),Fs);          
            end
            per_avg = mean(per_avg,1);
            per_avg = per_avg';
            [Y, I] = max(per_avg);
            signal_est_periodogram_avg(j,i) = Y;
            dist_est_periodogram_avg(k,j,i) = F_avg(I)*c/alpha/2;

            solution_avg_lse = pnoise_fit(per_avg,F_avg,[N/4/avgnum tauc snoise]);
            dist_est_periodogram_avg_lse(k,j,i) = c*solution_avg_lse(1)*(F_avg(2)-F_avg(1))/alpha/2;

            solution_avg_mle = mlelorentz(per_avg,F_avg,[N/4/avgnum tauc snoise]);
            dist_est_periodogram_avg_mle(k,j,i) = c*solution_avg_mle(1)*(F_avg(2)-F_avg(1))/alpha/2;
            ...
            [per_correct,F] = periodogram(tempx_windowed_correct,window(@rectwin,length(tempx_windowed_correct)),length(tempx_windowed_correct),Fs);
            [Y, I] = max(per_correct);
            signal_est_periodogram_correct(j,i) = Y;
            dist_est_periodogram_correct(k,j,i) = F(I)*c/alpha/2;

            [S_correct, F] = pmusic(tempx_windowed_correct,6,freq,Fs);
            [Y, I] = max(S_correct);
        ...
        std_est_periodogram_avg(k,j) = std(dist_est_periodogram_avg(k,j,:));
        std_est_periodogram_avg_lse(k,j) = std(dist_est_periodogram_avg_lse(k,j,:));
        std_est_periodogram_avg_mle(k,j) = std(dist_est_periodogram_avg_mle(k,j,:));
