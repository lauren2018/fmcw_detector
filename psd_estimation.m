...
for k=1:length(Prx)
    for j=1:length(distance)
        for i=1:iter
            ...
            solution_lse = pnoise_fit(per,F,[N/4 tauc 1e-9]);
            dist_est_periodogram_lse(k,j,i) = c*solution_lse(1)*(F(2)-F(1))/alpha/2;

            solution_mle = mlelorentz(per,F,[N/4 tauc 1e-9]);
            ...
            solution_avg_lse = pnoise_fit(per_avg,F,[N/4/avgnum tauc 1e-9]);
            dist_est_periodogram_avg_lse(k,j,i) = c*solution_avg_lse(1)*(F(2)-F(1))/alpha/2;

            solution_avg_mle = mlelorentz(per_avg,F,[N/4/avgnum tauc 1e-9]);
